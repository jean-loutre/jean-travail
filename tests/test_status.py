...
def test_status(cli: Cli) -> None:
    assert cli("status") == "Idle: 00:00\n"

    cli("next")
    assert cli("status") == "Work: 25:00\n"

    cli.tick(67)
    assert cli("status") == "Work: 23:53\n"

    cli.tick(25 * 60)
    assert cli("status") == "Work: -1:07\n"

    cli("next")
    assert cli("status") == "Pause: 05:00\n"

    cli.tick(67)
    assert cli("status") == "Pause: 03:53\n"

    cli.tick(5 * 60)
    assert cli("status") == "Pause: -1:07\n"


def test_work_duration_parameter(cli: Cli) -> None:
    with cli.config(work_duration="30"):
        cli("next")
        assert cli("status") == "Work: 30:00\n"

        with cli.environment(JTRAVAIL_WORK_DURATION="40"):
            cli("stop", "next")
            assert cli("status") == "Work: 40:00\n"

            cli("stop", "next")
            assert cli("status -w 50") == "Work: 50:00\n"
            assert cli("status --work-duration 50") == "Work: 50:00\n"


def test_pause_duration_parameter(cli: Cli) -> None:
    with cli.config(pause_duration="30"):
        cli("next", "next")
        assert cli("status") == "Pause: 30:00\n"

        with cli.environment(JTRAVAIL_PAUSE_DURATION="40"):
            cli("stop", "next", "next")
            assert cli("status") == "Pause: 40:00\n"

            cli("stop", "next", "next")
            assert cli("status -p 50") == "Pause: 50:00\n"
            assert cli("status --pause-duration 50") == "Pause: 50:00\n"
